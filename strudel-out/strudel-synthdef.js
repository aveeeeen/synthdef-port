 ...
 * - Compatible with initStrudel({ prebake: () => registerSynthDefs() })
 * - Protected against Strudel mini-notation transpiler string conversions
 */

// ============================================================================
// Safe Helpers for Strudel Transpiler & Scope Compatibility
// ============================================================================

/**
 * Resolves a string even if Strudel's transpiler wrapped it into a Pattern object (m(...))
 */
const resolveString = (val) => {
  if (typeof val === "string") return val;
  if (val && typeof val === "object") {
    if (typeof val.value === "string") return val.value;
    if (Array.isArray(val.firstCycleValues) && typeof val.firstCycleValues[0] === "string") {
      return val.firstCycleValues[0];
    }
  }
  return String(val);
};
...
/**
 * Obtains the active registerSound function from current environment
 */
const getSafeRegisterSound = () => {
  const reg = typeof registerSound === "function"
    ? registerSound
    : (typeof globalThis !== "undefined" && typeof globalThis.registerSound === "function"
      ? globalThis.registerSound
      : (typeof window !== "undefined" && typeof window.registerSound === "function" ? window.registerSound : null));

  if (!reg) {
    throw new Error("registerSound is not defined. Ensure initStrudel() has been called before registering synths.");
  }

  return (key, onTrigger, options = {}) => {
    return reg(resolveString(key), onTrigger, options);
  };
};

/**
 * Obtains the active Web Audio Context safely
 */
const getSafeAudioContext = () => {
  if (typeof getAudioContext === "function") return getAudioContext();
  if (typeof globalThis !== "undefined" && typeof globalThis.getAudioContext === "function") return globalThis.getAudioContext();
  if (typeof window !== "undefined" && typeof window.getAudioContext === "function") return window.getAudioContext();
  const AC = typeof AudioContext !== "undefined" ? AudioContext : (typeof webkitAudioContext !== "undefined" ? webkitAudioContext : null);
  if (AC) return new AC();
  throw new Error("getAudioContext is not available");
};

/**
 * Safe OscillatorNode constructor / factory that unwraps transpiled type strings
 */
function SafeOscillatorNode(ctx, options = {}) {
  const opts = { ...options };
  if (opts.type) {
    opts.type = resolveString(opts.type);
  }
  const RealOsc = (typeof OscillatorNode !== "undefined" && OscillatorNode !== SafeOscillatorNode)
    ? OscillatorNode
    : (globalThis.OscillatorNode || (typeof window !== "undefined" ? window.OscillatorNode : null));
  if (!RealOsc) throw new Error("OscillatorNode is not available");
  return new RealOsc(ctx, opts);
}

/**
 * Safe BiquadFilterNode constructor / factory that unwraps transpiled type strings
 */
function SafeBiquadFilterNode(ctx, options = {}) {
  const opts = { ...options };
  if (opts.type) {
    opts.type = resolveString(opts.type);
  }
  const RealFilter = (typeof BiquadFilterNode !== "undefined" && BiquadFilterNode !== SafeBiquadFilterNode)
    ? BiquadFilterNode
    : (globalThis.BiquadFilterNode || (typeof window !== "undefined" ? window.BiquadFilterNode : null));
  if (!RealFilter) throw new Error("BiquadFilterNode is not available");
  return new RealFilter(ctx, opts);
}

/**
 * Safe GainNode constructor / factory
 */
function SafeGainNode(ctx, options = {}) {
  const RealGain = (typeof GainNode !== "undefined" && GainNode !== SafeGainNode)
    ? GainNode
    : (globalThis.GainNode || (typeof window !== "undefined" ? window.GainNode : null));
  if (!RealGain) throw new Error("GainNode is not available");
  return new RealGain(ctx, options);
}

const createSafeOscillatorNode = SafeOscillatorNode;
const createSafeBiquadFilterNode = SafeBiquadFilterNode;

// ============================================================================
// Generator Nodes & DSP Utilities
// ============================================================================
...
const whiteNoiseNode = (ctx, gain = 0.4) => {
  ...
  const gainNode = new (globalThis.GainNode || window.GainNode)(audioCtx, { gain: gain });
...
const waveShaperNode = (ctx, type = "tanh", amount = 1) => {
  const WSN = globalThis.WaveShaperNode || window.WaveShaperNode;
  const shaper = new WSN(ctx, { oversample: "2x" });
  if (type === "tanh") {
...
const combFilterNode = (ctx, delaySec = 0.02, feedback = 0.5) => {
  const GN = globalThis.GainNode || window.GainNode;
  const DN = globalThis.DelayNode || window.DelayNode;
  const input = new GN(ctx, { gain: 1 });
  const delay = new DN(ctx, { delayTime: Math.min(delaySec, 1.0) });
  const feedbackGain = new GN(ctx, { gain: feedback });
  const output = new GN(ctx, { gain: 1 });
...
const createKlangNode = (ctx, baseFreq, ratios, amps, time, dur) => {
  const GN = globalThis.GainNode || window.GainNode;
  const outGain = new GN(ctx, { gain: 1 });
  const oscs = [];
  const gains = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const osc = createSafeOscillatorNode(ctx, { type: "sine", frequency: f });
    const g = new GN(ctx, { gain: Number(amps[i]) });
    osc.connect(g);
    g.connect(outGain);
    osc.start(time);
...
// ============================================================================
// Main Registration Function (Register all 51 Synthesizers)
// ============================================================================

export function registerSynthDefs(customRegisterSound, customGetAudioContext) {
  const registerSound = customRegisterSound
    ? ((key, onTrigger, options = {}) => customRegisterSound(resolveString(key), onTrigger, options))
    : getSafeRegisterSound();
  const getAudioContext = customGetAudioContext || getSafeAudioContext;
  const OscillatorNode = SafeOscillatorNode;
  const BiquadFilterNode = SafeBiquadFilterNode;
  const GainNode = SafeGainNode;
...
export default registerSynthDefs;

// Auto-register when loaded in browser environments where registerSound is globally available
if (typeof globalThis !== "undefined") {
  globalThis.registerSynthDefs = registerSynthDefs;
